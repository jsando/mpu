...
- Add frame pointer, sav/rst, pop #imm, auto-fp relative calculations
- fp-relative could use byte ops, giving +127/-128 offsets for 36% savings on size, pop #imm can be byte, and sav #
- Fix sp so it always points to the last byte written to the stack, ie it can start at 0000 and the first push is at 0xffff
- branches can be byte offsets as well
